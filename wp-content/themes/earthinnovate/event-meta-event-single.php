...
<div class="entry-meta eventorganiser-event-meta">
	...
	<?php if( eo_is_all_day() ){
		...
		$date_format = 'l, F j, Y – ' . get_option('time_format');
	} ?>
	...
	<!-- Event details -->
		...
		<?php if( eo_reoccurs() ):?>
			...
			<?php if($next): ?>
				...
				<?php printf('<p>'.__('This event is running from %1$s to %2$s. It is next occurring on %3$s','eventorganiser').'.</p>', eo_get_schedule_start('F j, Y'), eo_get_schedule_last('F j, Y'), $next);?>
	...
	<ul class="eo-event-meta">
		...
		<?php if( !eo_reoccurs() ) : ?>
				...
				<?php if (eo_is_all_day()): ?><li><?php _ex('This is an All-Day Event.', 'events page', 'earthinnovate'); ?></li>
				<?php else : ?><li><strong><?php _e('End', 'eventorganiser') ;?>:</strong> <?php eo_the_end($date_format); ?> </li>
				<?php endif;
		endif; ?>

		<?php if( eo_get_venue() ) : ?>
			<li><strong><?php _e('Venue','eventorganiser'); ?>:</strong> <?php eo_venue_name(); ?></li>
		<?php endif; ?>
